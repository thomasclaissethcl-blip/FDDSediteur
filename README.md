...
## Afficher ou retirer une carte personnage

Il n’existe plus deux types d’articles séparés. Un article reste un article. Il affiche une carte personnage uniquement lorsque l’option **Afficher une carte personnage** est cochée.

Si une carte existe déjà dans l’article chargé, l’éditeur l’identifie et coche cette option afin que vous puissiez décider si elle doit être conservée ou retirée.

Quand l’option est cochée, la zone **Carte de personnage** apparaît. Elle permet de renseigner une carte standardisée, cohérente avec le style du site.

Quand l’option est décochée, aucune carte personnage n’est générée dans l’article publié. Les informations de carte peuvent rester conservées dans les données éditoriales, mais elles ne sont pas affichées sur le site.
...
Lorsque l’article est généré, la carte est placée automatiquement au bon endroit dans la page publique uniquement si l’option **Afficher une carte personnage** est cochée.
